...
def main():
    ...
    for i, config in enumerate(configs, 1):
        ...
        behavior_notes = {
            1: "MLP struggles with visual input, poor spatial understanding",
            2: "CNN baseline with optimal exploration schedule",
            3: "Higher LR for faster convergence with CNN architecture",
            4: "Higher gamma + larger batch for long-term reward optimization"
        }
        results.append({
            "Config": i,
    ...
    print(f"\n{'='*80}")
    print("BEHAVIOR OBSERVATIONS")
    print(f"{'='*80}")
    for _, row in df.iterrows():
        print(f"Config {row['Config']}: {row['Behavior Notes']}")
